...
export function BackendPrepWorkspace() {
  ...
  const [fastApiPanel, setFastApiPanel] = useState<"commands" | "concepts" | "questions">(
    "questions"
  );
  const [openQuestion, setOpenQuestion] = useState<string | null>(
    `nodejs-${nodeInterviewSections[0]?.section}-0`
  );
            ...
            <div className="space-y-6">
              <div className="flex flex-wrap gap-3">
                {[
                  ...
                  const isActive = nodePanel === panel.id;
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        setNodePanel(panel.id as "commands" | "concepts" | "questions")
                      }
              ...
              {nodePanel === "commands" ? (
                <section className="rounded-[28px] border border-[var(--color-border)] bg-[color-mix(in_srgb,var(--color-card)_78%,transparent)] p-6">
                  ...
                  <p className="mt-2 text-sm text-[var(--color-text-muted)]">
                    Baseline Node.js project setup commands for interview review.
                  </p>
              ...
              {nodePanel === "concepts" ? (
                <section className="rounded-[28px] border border-[var(--color-border)] bg-[color-mix(in_srgb,var(--color-card)_78%,transparent)] p-6">
                  ...
                  <p className="mt-2 text-sm text-[var(--color-text-muted)]">
                    Fast review topics for senior backend, runtime, and production architecture
                    interviews.
                  </p>
              ...
              {nodePanel === "questions" ? (
                <section className="rounded-[28px] border border-[var(--color-border)] bg-[color-mix(in_srgb,var(--color-card)_78%,transparent)] p-6">
                  ...
                  <p className="mt-2 text-sm text-[var(--color-text-muted)]">
                    Senior-level Node.js questions grouped around fundamentals, runtime internals,
                    production operations, and architecture patterns.
                  </p>

                  <div className="mt-6 space-y-5">
                    {nodeInterviewSections.map((section) => (
                      <div
                        key={section.section}
                      ...
                      >
                        ...
                        <div className="mt-4 space-y-3">
                          {section.questions.map((question, index) => {
                            const itemKey = `nodejs-${section.section}-${index}`;
            ...
            <div className="space-y-6">
              <div className="flex flex-wrap gap-3">
                {[
                  ...
                  const isActive = fastApiPanel === panel.id;
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        setFastApiPanel(panel.id as "commands" | "concepts" | "questions")
                      }
                      className={`rounded-2xl border px-4 py-2.5 text-sm font-semibold transition duration-200 ${
                        isActive
                          ? "border-[var(--color-primary-blue)] bg-[color-mix(in_srgb,var(--color-primary-blue)_18%,transparent)] text-white"
              ...
              {fastApiPanel === "commands" ? (
                <section className="rounded-[28px] border border-[var(--color-border)] bg-[color-mix(in_srgb,var(--color-card)_78%,transparent)] p-6">
                  <h2 className="text-2xl font-semibold tracking-tight text-white">
                    FastAPI Commands
                  </h2>
                  <p className="mt-2 text-sm text-[var(--color-text-muted)]">
                    Baseline FastAPI project setup commands for interview review.
                  </p>
                  <div className="mt-5 overflow-hidden rounded-[24px] border border-[var(--color-border)] bg-[#09111f]">
                    <div className="flex items-center justify-between border-b border-[var(--color-border)] px-4 py-3">
                      <span className="text-xs font-semibold uppercase tracking-[0.2em] text-[var(--color-text-muted)]">
              ...
              {fastApiPanel === "concepts" ? (
                <section className="rounded-[28px] border border-[var(--color-border)] bg-[color-mix(in_srgb,var(--color-card)_78%,transparent)] p-6">
                  <h2 className="text-2xl font-semibold tracking-tight text-white">
                    FastAPI Core Concepts
                  </h2>
                  <p className="mt-2 text-sm text-[var(--color-text-muted)]">
                    Fast review topics for senior Python backend and API architecture interviews.
                  </p>
                  <div className="mt-5 flex flex-wrap gap-3">
                    {activeTopic.concepts.map((concept) => (
                      <span
              ...
              {fastApiPanel === "questions" ? (
                <section className="rounded-[28px] border border-[var(--color-border)] bg-[color-mix(in_srgb,var(--color-card)_78%,transparent)] p-6">
                  <h2 className="text-2xl font-semibold tracking-tight text-white">
                    FastAPI Interview Question Bank
                  </h2>
                  <p className="mt-2 text-sm text-[var(--color-text-muted)]">
                    Senior-level FastAPI questions grouped around ASGI internals, async design, and
                    production backend systems.
                  </p>

                  <div className="mt-6 space-y-5">
                    {fastApiInterviewSections.map((section) => (
                      <div
                        key={section.section}
                        className="rounded-[26px] border border-[var(--color-border)] bg-slate-950/25 p-5"
                      >
                        ...
                        <div className="mt-4 space-y-3">
                          {section.questions.map((question, index) => {
                            const itemKey = `fastapi-${section.section}-${index}`;
                            const isOpen = openQuestion === itemKey;
